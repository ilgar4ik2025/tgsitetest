(() => {
  ...
  function openProject(id){
    ...
    // Галерея превью (простая горизонтальная прокрутка)
    let gal = qs('#pg-slider .slides'); if (!gal){ const wrap=document.createElement('div'); wrap.id='pg-slider'; wrap.className='slider'; wrap.innerHTML='<div class="slides"></div>'; qs('.pd-meta')?.insertAdjacentElement('afterend', wrap); gal = qs('.slides', wrap); }
    gal.innerHTML=''; (d.images||[]).forEach(url=>{ const s=document.createElement('div'); s.className='slide'; s.innerHTML = `<div class="img-slide" style="background:#111;border-radius:12px;height:220px;background-position:center;background-repeat:no-repeat;background-size:contain;background-image:url(${nu(url)})"></div>`; gal.appendChild(s); });
  }
  ...
  function renderReviews(){ /* компактная версия */ }
